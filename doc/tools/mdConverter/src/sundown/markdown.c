...
parse_tab( struct buf *ob, struct sd_markdown *rndr, uint8_t *data, size_t size )
{
   ...
   {
      i += parse_list( ob, rndr, data, size, 0, _tabPage );
   }
   else if( prefix_oli( data, size ) )
   {
      i += parse_list( ob, rndr, data, size, MKD_LIST_ORDERED, _tabPage );
   }
...
void
sd_markdown_render( struct buf *ob, const uint8_t *document, size_t doc_size, struct sd_markdown *md, int tabPage )
{
#define MARKDOWN_GROW(x) ((x) + ((x) >> 1))
   ...
   while( beg < doc_size ) /* iterating over lines */
      if( _tabPage && is_tab_page( document, beg, doc_size, &end, md->refs, text ) )
         beg = end;

...
function _execSql( hostName, port, user, passwd, cmd, installPath, sql, database )
{
   var result = null ;
                             ...
                             mysqlUser, mysqlPasswd, installPath )
{
   ...
   try
   {
      _execSql( hostName, mysqlPort, mysqlUser, mysqlPasswd, cmd, installPath, sql ) ;
   }
   catch( e )
   {
      var error = _getErrorResult( "Failed to set mariadb variables, detail: ?" ) ;
      PD_LOGGER.log( PDERROR, error ) ;
      throw error ;
   }
}
...
function _resetConfigOffline( PD_LOGGER, hostName, agentPort, path )
{
   ...
   PD_LOGGER.log( PDEVENT, "Save mariadb configs" ) ;
   ...
   try
   {
      ...
      file.disableItem( 'mysqld', 'sequoiadb_user' ) ;
      file.disableItem( 'mysqld', 'sequoiadb_password' ) ;

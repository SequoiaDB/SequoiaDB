...
static const char tabEscape[] = "\\t";
static const char returnEscape[] = "\\r";
static const char newlineEscape[] = "\\n";
static const char formfeedEscape[] = "\\f";
static const char backspaceEscape[] = "\\b";
...
write_string(JSContext *cx, StringBuffer &sb, const jschar *buf, uint32 len)
{
    ...
    for (i = 0; i < len; ++i) {
        if (buf[i] == quote || buf[i] == backslash) {
            ...
            if (!sb.append(&buf[mark], i - mark) ||
                !sb.append(returnEscape)) {
                return JS_FALSE;
            }
            mark = i + 1;
        } else if ( buf[i] == '\n' ) {
            if (!sb.append(&buf[mark], i - mark) ||
                !sb.append(newlineEscape)) {
                return JS_FALSE;
            }
            mark = i + 1;
        } else if ( buf[i] == '\f' ) {
            if (!sb.append(&buf[mark], i - mark) ||
                !sb.append(formfeedEscape)) {
                return JS_FALSE;
            }
            mark = i + 1;
        } else if ( buf[i] == '\b' ) {
            if (!sb.append(&buf[mark], i - mark) ||
                !sb.append(backspaceEscape)) {
                return JS_FALSE;
            }
            mark = i + 1;

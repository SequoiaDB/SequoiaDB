...
//    }
     ...
     * Note:
     * This method is not supported in package org.springframework.data.mongodb.assist
     * @return
     */
    ...
    public DBObject next() {
        _num++;
        return (DBObject)_resultIterator.next();
    }
    ...
    public DBObject curr(){
        return (DBObject)_resultIterator.current();
    }
    ...
    public List<DBObject> toArray( int max ) {
        while(max > _all.size() && _resultIterator.hasNext()) {
            _all.add((DBObject)_resultIterator.next());
        }
        return _all;
    }

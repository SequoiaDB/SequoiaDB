 ...
 *   {@code fsync}. Prior to SequoiaDB 2.6 this option was ignored if the server was running without journaling.  Starting with SequoiaDB 2.6
...
public class WriteConcern implements Serializable {
     ...
     * then catch and ignore any exceptions of type SequoiadbSocketException.
     * @see com.sequoiadb.WriteConcern#UNACKNOWLEDGED
     * @see com.sequoiadb.SequoiadbSocketException
     */
    @Deprecated
     ...
     */
    public BasicBSONObject getCommand() {
        BasicBSONObject command = new BasicBSONObject("getlasterror", 1);
    ...
    private void addJ(final BasicBSONObject document) {
        if (_j) {
            document.put("j", true);
        }
    }

    private void addFSync(final BasicBSONObject document) {
        if (_fsync) {
            document.put("fsync", true);
        }
    }

    private void addWTimeout(final BasicBSONObject document) {
        if (_wtimeout > 0) {
     ...
     * @see com.sequoiadb.WriteConcern#ERRORS_IGNORED
     */
    @Deprecated

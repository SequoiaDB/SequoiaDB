...
import org.springframework.data.sequoiadb.core.aggregation.Fields.AggregationField;
...
/**
 * An {@code Aggregation} is a representation of a list of aggregation steps to be performed by the SequoiaDB Aggregation
 * Framework.
 * 
...
public class Aggregation {
	 ...
	 * supported in SequoiaDB version 2.6+.
	 * 
	...
	/**
	 * Converts this {@link Aggregation} specification to a {@link BSONObject}.
	 * 
	 * @param inputCollectionName the name of the input collection
	 * @return the {@code BSONObject} representing this aggregation
	 */
	public BSONObject toDbObject(String inputCollectionName, AggregationOperationContext rootContext) {

		AggregationOperationContext context = rootContext;
		List<BSONObject> operationDocuments = new ArrayList<BSONObject>(operations.size());
		...
		BSONObject command = new BasicBSONObject("aggregate", inputCollectionName);
	...
	/**
	 * Describes the system variables available in SequoiaDB aggregation framework pipeline expressions.
	 * 

	 * @see http://docs.sequoiadb.org/manual/reference/aggregation-variables
	 */
	enum SystemVariable {

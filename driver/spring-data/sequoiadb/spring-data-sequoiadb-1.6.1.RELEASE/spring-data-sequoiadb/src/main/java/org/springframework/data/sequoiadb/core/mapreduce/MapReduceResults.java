...
public class MapReduceResults<T> implements Iterable<T> {
	 ...
	 */
	public MapReduceResults(List<T> mappedResults, BSONObject rawResults) {
	...
	private MapReduceTiming parseTiming(BSONObject rawResults) {

		BSONObject timing = (BSONObject) rawResults.get("timing");
	 ...
	 * @return
	 */
	private Long getAsLong(BSONObject source, String key) {
		Object raw = source.get(key);
		return raw instanceof Long ? (Long) raw : (Integer) raw;
	}

	/**
	 * Parses the raw {@link BSONObject} result into a {@link MapReduceCounts} value object.
	 * 
	 * @param rawResults
	 * @return
	 */
	private MapReduceCounts parseCounts(BSONObject rawResults) {

		BSONObject counts = (BSONObject) rawResults.get("counts");
	...
	/**
	 * Parses the output collection from the raw {@link BSONObject} result.
	 * 
	 * @param rawResults
	 * @return
	 */
	private String parseOutputCollection(BSONObject rawResults) {
		...
		return resultField instanceof BSONObject ? ((BSONObject) resultField).get("collection").toString() : resultField
				.toString();
	}
}

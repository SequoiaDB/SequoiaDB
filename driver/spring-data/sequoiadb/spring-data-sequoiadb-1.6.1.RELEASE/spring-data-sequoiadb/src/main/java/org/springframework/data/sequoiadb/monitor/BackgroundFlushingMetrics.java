...
public class BackgroundFlushingMetrics extends AbstractMonitor {
	...
	public BackgroundFlushingMetrics(Sdb sdb) {
		this.sdb = sdb;
	}
	...
	private <T> T getFlushingData(String key, Class<T> targetClass) {
		BSONObject mem = (BSONObject) getServerStatus().get("backgroundFlushing");
		return (T) mem.get(key);
	}

	private Date getLast() {
		BSONObject bgFlush = (BSONObject) getServerStatus().get("backgroundFlushing");

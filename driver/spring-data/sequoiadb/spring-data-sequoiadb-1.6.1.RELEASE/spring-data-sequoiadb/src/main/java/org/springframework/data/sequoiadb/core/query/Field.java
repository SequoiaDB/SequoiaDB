...
public class Field {
	...
	public Field include(String key, int include) {
		this.key = key;
		if (include == 0) {
			fields.put(key, new BasicBSONObject("$include", Integer.valueOf(0)));
		} else {
			fields.put(key, new BasicBSONObject("$include", Integer.valueOf(1)));
		}
		return this;
	}
	...
	public Field defaultValue(String key, Object defaultValue) {
		this.key = key;
		fields.put(key, new BasicBSONObject("$default", defaultValue));
		return this;
	}
	...
	public Field elemMatch(String key, Criteria elemMatchCriteria) {
		...
		this.key = key;
		fields.put(key, new BasicBSONObject("$elemMatch", elemMatchCriteria.getCriteriaObject()));
		return this;
	}
	...
	public Field elemMatchOne(String key, Criteria elemMatchCriteria) {
		Assert.notNull(elemMatchCriteria, "Criteria must not be null or empty!");
		this.key = key;
		fields.put(key, new BasicBSONObject("$elemMatchOne", elemMatchCriteria.getCriteriaObject()));
		return this;
	}
	...
	private boolean isCommandOperation(BSONObject obj) {
		Assert.notNull(obj);
	...
	private Field funcOperator(String operator, List<Integer> list) {
		if (fields.containsKey(this.key)) {
			Object obj = fields.get(this.key);
			if (obj instanceof BSONObject && isCommandOperation((BasicBSONObject)obj)) {
				BSONObject doc = (BasicBSONObject)obj;
				doc.put(operator, list);
			} else {
				fields.put(this.key, new BasicBSONObject(operator, list));
			}
		}
		return this;
	}
	...
	private Field funcOperator(String operator, Number num) {
		if (fields.containsKey(this.key)) {
			...
			if (obj instanceof BSONObject && isCommandOperation((BasicBSONObject)obj)) {
				...
				fields.put(this.key, new BasicBSONObject(operator, num));
			}
		}
		return this;
	}

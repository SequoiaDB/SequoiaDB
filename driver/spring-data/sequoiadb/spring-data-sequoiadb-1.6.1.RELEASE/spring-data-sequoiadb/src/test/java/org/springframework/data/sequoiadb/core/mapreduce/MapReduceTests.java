...
import static org.springframework.data.sequoiadb.core.mapreduce.MapReduceOptions.*;
...
/**
 * Integration test for {@link SequoiadbTemplate}'s Map-Reduce operations
 * 
...
public class MapReduceTests {
	...
	public void testForDocs() {
		createMapReduceData();
		MapReduceResults<ValueObject> results = sequoiadbTemplate.mapReduce("jmr1", mapFunction, reduceFunction,
				ValueObject.class);
	...
	public void testIssue260() {
		...
		MapReduceResults<ContentAndVersion> results = sequoiadbTemplate.mapReduce("jmr2", map, reduce,
		...
		for (ContentAndVersion cv : results) {
			...
			if (cv.getId().equals("sequoiadb How-To")) {
				assertEquals(2, cv.getValue().longValue());
			}
			size++;
		}
	...
	public void testIssue260Part2() {
		...
		String reduce = "function (key, values) { return Math.max.apply(Math, values); }";
		MapReduceResults<NumberAndVersion> results = sequoiadbTemplate.mapReduce("jmr2", map, reduce,
	...
	private void createContentAndVersionData() {
		/*
		{ "_id" : 1, "document_id" : "sequoiadb How-To", "author" : "Amos King", "content" : "...", "version" : 1 }
		{ "_id" : 2, "document_id" : "sequoiadb How-To", "author" : "Amos King", "content" : "...", "version" : 1.1 }
		...
		cv1.setDocumentId("sequoiadb How-To");
		...
		cv2.setDocumentId("sequoiadb How-To");
	...
	public void testMapReduceInlineWithScope() {
		...
		MapReduceResults<ValueObject> results = sequoiadbTemplate.mapReduce("jmr1", mapWithExcludeFunction, reduceFunction,
	...
	public void testMapReduceExcludeQuery() {
		...
		MapReduceResults<ValueObject> results = sequoiadbTemplate.mapReduce(query, "jmr1", mapFunction, reduceFunction,
				ValueObject.class);
...
//		assertEquals(1, m.get("d").intValue());
	...
	private void performMapReduce(boolean inline, boolean withQuery) {
		...
		if (inline) {
			if (withQuery) {
				results = sequoiadbTemplate.mapReduce(new Query(), "jmr1", "classpath:map.js", "classpath:reduce.js",
						ValueObject.class);
			} else {
				results = sequoiadbTemplate.mapReduce("jmr1", mapFunction, reduceFunction, ValueObject.class);
			}
		} else {
			if (withQuery) {
				results = sequoiadbTemplate.mapReduce(new Query(), "jmr1", mapFunction, reduceFunction,
						options().outputCollection("jmr1_out"), ValueObject.class);
			} else {
				results = sequoiadbTemplate.mapReduce("jmr1", mapFunction, reduceFunction,
	...
	private void createMapReduceData() {
		DBCollection c = sequoiadbTemplate.getDb().getCollection("jmr1");
		c.save(new BasicBSONObject("x", new String[] { "a", "b" }));
		c.save(new BasicBSONObject("x", new String[] { "b", "c" }));
		c.save(new BasicBSONObject("x", new String[] { "c", "d" }));
	}

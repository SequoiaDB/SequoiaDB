...
import org.springframework.data.sequoiadb.core.WriteResultChecking;
import org.springframework.data.sequoiadb.core.index.GeoSpatialIndexType;
...
public class GeoSpatialIndexTests extends AbstractIntegrationTests {
	...
	@Autowired private SequoiadbTemplate template;
		...
		return template.execute(entityType, new CollectionCallback<Boolean>() {
			...
			public Boolean doInCollection(DBCollection collection) throws BaseException, DataAccessException {
				...
				for (BSONObject indexInfo : collection.getIndexInfo()) {
					...
					BSONObject keys = (BSONObject) indexInfo.get("key");

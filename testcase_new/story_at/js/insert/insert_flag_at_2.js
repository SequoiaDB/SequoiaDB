 ...
 * 环境准备：任意正常集群
 * 测试场景：
 *    批量记录插入场景下，测试insert flag:SDB_INSERT_CONTONDUP_ID 及 insert option：ContOnDupID的行为是否符合预期。
 * 测试步骤：
 *    1. 创建cs和cl，cl默认创建'$id'索引
 *    2. cl为'a'字段创建唯一索引'aIdx'
 *    3. 向cl中插入一条记录rec
 *    4. 设置 insert flag: SDB_INSERT_CONTONDUP_ID，向cl中插入三条记录，校验返回结果
 *       （三条记录中的第三条与rec有相同'_id'字段和'a'字段的记录，其余记录没有冲突字段）
 *    5. 清空cl
 *    6. 向cl中插入记录rec
 *    7. 设置 insert flag: SDB_INSERT_CONTONDUP_ID，向cl中插入三条记录，校验返回结果
 *       （三条记录中的第三条与rec有相同'a'字段的记录，其余记录没有冲突字段）
 *    8. 清空cl
 *    9. 向cl中插入记录rec
 *    10. 设置 insert option: {ContOnDupID:true}，向cl中插入三条记录，校验返回结果
 *       （第一条记录与rec的'_id'字段相同；第二条'_id'字段不同'a'字段相同；第三条没有冲突字段）
 *    11. 清空cl
 *    12. 删除cs
 * 期望结果：
 *    1. 步骤4中的前两条记录未见冲突正常插入，第三条记录因'_id'字段重复被忽略，
 *       返回结果InsertedNum为2，DuplicatedNum为1，总记录数为3
 *    2. 步骤7中的前两条记录未见冲突正常插入，第三条记录因'a'字段重复而报错-38，
 *       插入的记录数为2，总记录数为3
 *    3. 步骤10的第一条记录因'_id'重复而被忽略，第二条记录因'a'字段重复而报错-38,
 *       插入的记录数为0，总记录数为1
 *    
 **************************************************************************************************/
 ...
 function test() {
   ...
   data.push( r2 );
   data.push( r3 );
   var res = cl.insert( data, SDB_INSERT_CONTONDUP_ID ).toObj();
   ...
   assert.tryThrow( SDB_IXM_DUP_KEY, function () {
      res = cl.insert( data, SDB_INSERT_CONTONDUP_ID );
   });
   ...
   data.push( r3 );
   assert.tryThrow( SDB_IXM_DUP_KEY, function () {
      res = cl.insert( data, {ContOnDupID:true} );
   });
   recordCount = cl.find().count();

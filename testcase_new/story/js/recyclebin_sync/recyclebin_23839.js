/******************************************************************************
 * @Description   : seqDB-23839:SdbRecycle.list 带 cond/sel/sort 匹配所有项目
 * @Author        : Yang Qincheng
 * @CreateTime    : 2021.04.25
 * @LastEditTime  : 2022.06.23
 * @LastEditors   : liuli
 ******************************************************************************/
testConf.csName = CHANGEDPREFIX + "_cs_23839";
testConf.clName = COMMCLNAME + "_23839";
...
function test ()
{
   ...
   // list，cond匹配不存在的项目
   var cursor = recycleBin.list( { "RecycleName": "notExist" } );
   assert.equal( cursor.size(), 0 );
   // list，cond匹配存在的项目
   var cursor = recycleBin.list( { "RecycleName": recycleName1 } );
   assert.equal( cursor.size(), 1 );

   // list, sel匹配不存在的字段
   var cursor = recycleBin.list( { "RecycleName": recycleName1 }, { "notExist": "" } );
   assert.equal( cursor.next().toObj().notExist, "" );
   // list，sel匹配存在的项目
   var cursor = recycleBin.list( { "RecycleName": recycleName1 }, { "RecycleName": "" } );
   assert.equal( cursor.next().toObj().RecycleName, recycleName1 );

   // list, sort正序
   var cursor = recycleBin.list( { "OriginName": clFullName }, { "RecycleName": "" }, { "RecycleID": 1 } );

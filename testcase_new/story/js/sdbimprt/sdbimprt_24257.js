/******************************************************************************
 * @Description   : seqDB-24257:带--autodelchar导入带非默认字段分割符的记录
 * @Author        : luweikang
 * @CreateTime    : 2021.05.26
 * @LastEditTime  : 2021.02.03
 ******************************************************************************/

testConf.clName = COMMCLNAME + "cl_24257"; 
main( test )
...
function test ( testPara )
{
   ...
   var command = installDir + "bin/sdbimprt" +
      ...
      " -c " + COMMCSNAME + " -l " + testConf.clName + " --file " + importFile + " --fields 'str1,str2,str3' --job 1 --parsers 1 --insertnum 1 --delchar \\' --autodelchar true";
   // 执行导入
   cmd.run( command );
...
function prepareImportFile ()
{
   ...
   var filename = tmpFileDir + "test24257.csv";

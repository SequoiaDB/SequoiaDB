/******************************************************************************
 * @Description   : seqDB-24251 :: 带--autodelchar并指定--trim both
 * @Author        : luweikang
 * @CreateTime    : 2021.05.26
 * @LastEditTime  : 2021.02.03
 ******************************************************************************/

testConf.clName = COMMCLNAME + "cl_24251C"; 
main( test )
...
function test ( testPara )
{
   ...
   var command = installDir + "bin/sdbimprt" +
      ...
      " -c " + COMMCSNAME + " -l " + testConf.clName + " --file " + importFile + " --fields 'str1,str2,str3' --job 1 --parsers 1 --insertnum 1 --autodelchar true --trim both";
   // 执行导入
   cmd.run( command );
...
function prepareImportFile ()
{
   ...
   var filename = tmpFileDir + "test24251C.csv";

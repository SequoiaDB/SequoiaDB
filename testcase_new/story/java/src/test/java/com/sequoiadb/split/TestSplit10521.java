...
public class TestSplit10521 extends SdbTestBase {
    ...
    /*
     * 组合类型: 1.数组、int、timestamp 2.long、float、date、decimal 3._id+long
     */
    ...
    /**
     * 分区键字段分别覆盖多种数据类型，参考如下组合： 数组、int、timestamp
     */
    @Test
    public void testCombination1() {
        try {
            ...
            BSONObject option = ( BSONObject ) JSON.parse(
                    "{ShardingKey:{arrtype:1,age:1,timestamp:1},ShardingType:\"range\",Group:\""
                            + rgNames.get( 0 ) + "\"}" );
            this.cl = SplitUtils2.createCL( this.cs, this.clName1, option );
            insertData( 10 );

            BSONObject startCondition = ( BSONObject ) JSON.parse(
                    "{arrtype:2,age:-4,timestamp:{\"$timestamp\":\"2016-11-30-00.00.00.000000\"}}" );
            BSONObject endCondition = ( BSONObject ) JSON.parse(
                    "{arrtype:2,age:4,timestamp:{\"$timestamp\":\"2016-11-30-23.59.59.999999\"}}" );
    ...
    /**
     * 分区键字段分别覆盖多种数据类型，参考如下组合： long、float、date、decimal
     */
    @Test
    public void testCombination2() {
        try {
            ...
            List< String > rgNames = SplitUtils2.getDataRgNames( this.sdb );
            BSONObject option = ( BSONObject ) JSON.parse(
                    "{ShardingKey:{height:1,num:1,date:1,bdecimal:1},ShardingType:\"range\",Group:\""
                            + rgNames.get( 0 ) + "\"}" );
            ...
            // (BSONObject)
            // JSON.parse("{height:1,num:0.01,date:{\"$date\":\"2016-12-01\"},bdecimal:{\"$decimal\":\"12345.06789123456789012345000\"}}");
            BSONObject endCondition = new BasicBSONObject();
            // (BSONObject)
            // JSON.parse("{height:1,num:0.01,date:{\"$date\":\"2016-12-01\"},bdecimal:{\"$decimal\":\"12345.06789123456789012345255\"}}");
    ...
    /**
     * 分区键字段分别覆盖多种数据类型，参考如下组合： _id+long
     */
    @Test

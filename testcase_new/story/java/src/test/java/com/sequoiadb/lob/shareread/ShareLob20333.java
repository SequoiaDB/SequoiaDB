...
/**
 * @Description seqDB-20333 READ和SHARED_READ并发读写lob
 * @author luweikang
 * @Date 2019.8.26
 */

public class ShareLob20333 extends SdbTestBase {
    ...
    private String clName = "cl20333";
    private String mainCLName = "mainCL20333";
    private String subCLName = "subCL20333";
    ...
    public Object[][] generateCLName() {
        return new Object[][] {
                ...
                new Object[] { clName, 1024 },
                new Object[] { mainCLName, 1024 * 10 } };
    }
    ...
    public void testLob( String clName, int readSize ) {
        ...
        byte[] readLob4 = new byte[ readSize ];
        byte[] readLob5 = new byte[ readSize / 2 ];
        byte[] readLob6 = new byte[ readSize ];
        byte[] readLob7 = new byte[ readSize ];
        byte[] readLob8 = new byte[ readSize * 2 ];
        ...
        DBLob lob4 = cl4.openLob( id, DBLob.SDB_LOB_SHAREREAD );
        DBLob lob5 = cl5.openLob( id, DBLob.SDB_LOB_SHAREREAD );
        DBLob lob6 = cl6.openLob( id, DBLob.SDB_LOB_SHAREREAD );
        DBLob lob7 = cl7.openLob( id, DBLob.SDB_LOB_SHAREREAD );
        DBLob lob8 = cl8.openLob( id, DBLob.SDB_LOB_SHAREREAD );
        ...
        lob3.lockAndSeek( readSize * 3, readSize );
        lob3.read( readLob3 );
        byte[] expData3 = Arrays.copyOfRange( lobBuff, readSize * 3,
                readSize * 4 );
        RandomWriteLobUtil.assertByteArrayEqual( readLob3, expData3,
                "lob data is wrong" );

        // SHARED_READ读区域在READ读区域前并相交
        lob4.lockAndSeek( readSize * 4 - 1024, readSize );
        lob4.read( readLob4 );
        byte[] expData4 = Arrays.copyOfRange( lobBuff, readSize * 4 - 1024,
                readSize * 5 - 1024 );
        RandomWriteLobUtil.assertByteArrayEqual( readLob4, expData4,
                "lob data is wrong" );

        // SHARED_READ读区域在READ读区域内
        lob5.lockAndSeek( readSize * 4 + 1024, readSize / 2 );
        lob5.read( readLob5 );
        byte[] expData5 = Arrays.copyOfRange( lobBuff, readSize * 4 + 1024,
                readSize * 4 + 1024 + readSize / 2 );
        RandomWriteLobUtil.assertByteArrayEqual( readLob5, expData5,
                "lob data is wrong" );

        // SHARED_READ读区域在READ读区域后并相邻
        lob6.lockAndSeek( readSize * 5, readSize );
        lob6.read( readLob6 );
        byte[] expData6 = Arrays.copyOfRange( lobBuff, readSize * 5,
                readSize * 6 );
        RandomWriteLobUtil.assertByteArrayEqual( readLob6, expData6,
                "lob data is wrong" );

        // SHARED_READ读区域在READ读区域后并相交
        lob7.lockAndSeek( readSize * 4 - 1024, readSize );
        lob7.read( readLob7 );
        byte[] expData7 = Arrays.copyOfRange( lobBuff, readSize * 4 - 1024,
                readSize * 5 - 1024 );
        RandomWriteLobUtil.assertByteArrayEqual( readLob7, expData7,
                "lob data is wrong" );

        // SHARED_READ读区域包含READ读区域
        lob8.lockAndSeek( readSize * 3, readSize * 2 );
        lob8.read( readLob8 );
        byte[] expData8 = Arrays.copyOfRange( lobBuff, readSize * 3,
                readSize * 5 );
